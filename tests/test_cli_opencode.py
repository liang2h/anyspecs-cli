...
FIXTURE_SQLITE_STORAGE = (
    Path(__file__).resolve().parent / "fixtures" / "opencode" / "sqlite" / "storage"
)
...
def test_export_command_writes_sqlite_backed_opencode_markdown_and_sidecar(tmp_path):
    cli = AnySpecsCLI()
    cli.logger = DummyLogger()
    cli.extractors["opencode"].storage_root = FIXTURE_SQLITE_STORAGE
    ...
    assert result == 0
    export_file = tmp_path / "opencode-chat-sqlite-app-ses_sqlite_fixture_001.md"
    metadata_file = export_file.with_name(f"{export_file.name}.meta.json")
    ...
    assert "Tool Call: bash" in content
    assert "docs/sqlite.md" in content
    assert "Patch: sqlite-patch-001" in content
    assert "ignore me" not in content
    assert metadata["source"] == "opencode"
    assert metadata["session_id"] == "ses_sqlite_fixture_001"
    assert metadata["format"] == "markdown"
    assert metadata["project_name"] == "sqlite-app"
    assert metadata["dedupe_key"] == "opencode:ses_sqlite_fixture_001:markdown"
